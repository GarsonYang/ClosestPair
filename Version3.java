...
public class Version3
{
   ...
   public static void main (String[] args) throws FileNotFoundException
   {
      ...
      double[][] pointx = new double[max][2]; //use 2 dimesnion array to store x&y
      double[][] pointy = new double[max][2];
   	
      for (int i=0; input.hasNextDouble();i++) {
         pointx[i][0] = 0;
         pointx[i][1] = input.nextDouble();
         pointy[i][0] = pointx[i][0];
         pointy[i][1] = pointx[i][1];
         size++;
      }
      ...
      mergeSort(pointx, 0, size-1, 0); // sort by x
      mergeSort(pointy, 0, size-1, 1); // sort by y
      
      double result[] = new double[5];
      double startTime = System.nanoTime();
      result = closestPair(pointx, pointy, 0, size-1);
   ...
   private static double[] closestPair(double pointx[][], double pointy[][], int left, int right){
      double result[] = new double[5];//result[0] = d, result[1-4]=point1xy,point2xy
      
      //if there is only one point
      if(left >= right) {
         result[0] = Integer.MAX_VALUE;
         result[1]=pointx[left][0];
         result[2]=pointx[left][1];
      ...
      //get the smallest d
      result1 = closestPair(pointx, pointy, left, mid);
      result2 = closestPair(pointx, pointy, mid+1, right);
      ...
      for(int i=0; i<length;i++){
         if(pointy[i][0]>=pointx[mid][0]-result[0]||pointy[i][0]<=pointx[mid][0]+result[0]){
            temp[temp_size][0]=pointy[i][0];
            temp[temp_size][1]=pointy[i][1];
            temp_size++;
